...
---
# Šifrovací schéma
Šifrovací schéma je určeno trojicí $(Gen,Enc, Dec)$ pravděpodobnostních poly-time algoritmů a určením množiny zpráv $\mathscr M$, kde $|\mathscr M| > 1$. 
...
Zároveň se nám hodí, pro $c\in \mathscr C: \Pr[C =c] > 0$ 
$$
...
Máme-li platnou podmínku na pravděpodobnost rozeznatelnosti ciphertextů ke zprávám, tak zafixujeme-li si pravděpodobnostní distribuci na $\mathscr M$, tak můžeme očekávat $\forall c \in \mathscr C, \forall m \in \mathscr M: \Pr[C =c] >0$, tak i $\Pr[M=m] > 0$, protože byla-li by pravděpodobnost zprávy $0$, tak i víme $\Pr[M=m \mid C =c]$. $\forall c \in \mathscr C$ definujeme $p_{c} := \Pr[Enc_{K}(m) = c]$, z předpokladu a pozorování na začátku důkazu máme $\Pr[C=c \mid M =m'] = p_{c}$ pro všechna $m' \in \mathscr M$. Tedy dostáváme
$$
\begin{align*}
...
*Důkaz:* ($\impliedby$) Pro jednoduchost BÚNO $Enc$ je deterministické (veškerou náhodnost schováme do klíče). Zafixujeme libovolné $c \in \mathcal{C}, m\in \mathcal{M}$, nechť $k$ je unikátním klíčem, že $Enc_{k}(m)=c$, pak
$$
...
($\implies$) Předpokládejme perfektní tajnost, zafixujme si libovolné $c \in \mathcal{C}$, musí pak existovat $m^*$ pro kterou $\Pr[Enc_{K}(m^*)=c]\ne 0$. Dle [Alternativní definice perfektnosti schématu](#Alternativní%20definice%20perfektnosti%20schématu) dostáváme, že $\Pr[Enc_{K}(m)=c]\ne 0$ pro každé $m \in \mathcal{M}$. Pak z $\mathcal{M} = \{ m_{1},m_{2},\dots \}$ pro každou $m_{i}$ máme neprázdnou $\mathcal{K}_{i} \subset \mathcal{K}$ takovou, že $Enc_{k}(m_{i})=c \iff k \in \mathcal{K}_{i}$. Navíc pro $\mathcal{K}_{i},\mathcal{K}_{j},i \ne j$ musí být $\mathcal{K}_{i} \cap \mathcal{K}_{j} = \emptyset$, jinak by neplatila korektnost ($Dec_{k}$ je deterministické). Protože $|\mathcal{K}|= |\mathcal{M}|$, tak $\mathcal{K}_{i}$ může mít jen jeden klíč a tedy je splněna podmínka 2. Dle alternativní definice pro perfektní tajnost pro libovolné dvě zprávy $m_{i},m_{j}\in \mathcal{M}$ dostáváme
$$
