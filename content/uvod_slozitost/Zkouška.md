...
- (B4) Definice [základních tříd složitosti](Základní%20třídy%20složitosti) a důkaz $TIME(f(n))\subseteq SPACE(f(n))$.
- (B5) Definice [základních tříd složitosti](Základní%20třídy%20složitosti) a důkaz věty o vztahu prostoru a času ($(\forall L \in NSPACE(f(n)))(\exists c_{L}[L\in TIME(2^{c_{L}f(n)}) ])$).
