...
$\Pi_{k} \subseteq \Delta_{k+1}$ stejně jako u $\Sigma_{k}=co-\Pi_{k}$, jen otočíme odpovědi orákula a máme doplňek.
...
A\in \Sigma_{k}\iff B\in P, \text{polynom } p:x\in A\iff \exists^{p(|x|)}y_{1,}\forall^{p(|x|)}y_{2},\dots,Q^{p(|x|)}y_{n}: \\
...
- pro $j>0$ předpokládejme platnost pro $j-1$, pak 
$$
