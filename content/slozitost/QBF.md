...
Mějme $\varphi_{i}(C,C')= \exists C'', \forall D_{1},D_{2}((D_{1} = C \land D_{2}=C'') \lor(D_{1} = C'' \lor D_{2}=C'))\implies \varphi(D_{1},D_{2})$, pak $\varphi_{K(n)}(q_{init},q_{acc}) = 1 \iff x\in L$ a prostor pro $|\varphi_{i}|=O(K(n)+|\varphi_{i-1}|)$ a tedy $|\varphi_{K(n)}|\in O(K(n)^2)$.
