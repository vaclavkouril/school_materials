...
- Analyzujte k-cestný Mergesort v [cache](Cache.md)-aware modelu. Jaká je optimální volba $k$? ✅
- Formulujte [cache](Cache.md)-oblivious algoritmus pro transpozici čtvercové matice. Rozeberte časovou složitost a I/O složitost. ✅
- Popište systém hešovacích funkcí odvozený ze [skalárního součinu](Hash%20ze%20skalárního%20součinu.md). Dokažte, že je to 1-univerzální systém ze $\mathbb{Z}_p^k$ do $\mathbb{Z}_p$. ✅
- Popište systém hešovacích funkcí založených na [lineární kongruenci](Hash%20z%20lineárních%20kongruencí.md) . Dokažte, že je to 2-nezávislý systém ze $\mathbb{Z}_p$ do $[m]$ (můžete využít lemma o modulení, které zformulujte, ale nemusíte dokazovat). ✅
- Sestrojte [k-nezávislý systém hešovacích funkcí](k-nezávislý%20systém%20hešovacích%20funkcí.md) ze $\mathbb{Z}_p$ do $[m]$. Zdůvodněte k-nezávislost (můžete využít lemma o modulení, které zformulujte, ale nemusíte dokazovat). ✅
- Sestrojte 2-nezávislý systém hešující řetězce délky nejvýše $L$ nad abecedou $[a]$ do $[m]$ založený na polynomech, tedy „[Rolling hash](Rolling%20hash.md)“. Popište výhodu použití tohoto systému oproti jiným hešovacím funkcím. ✅
